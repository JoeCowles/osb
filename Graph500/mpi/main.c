...
int main(int argc, char **argv) {
  ...
  if (tg.data_in_file) {
    ...
    if (!is_opened) {
      MPI_File_open(MPI_COMM_WORLD, (char *)filename, mode, MPI_INFO_NULL,
                    &tg.edgefile);
      MPI_File_set_size(tg.edgefile,
                        ((FILE_Offset)tg.nglobaledges) * sizeof(packed_edge));
    }
  ...
  int64_t *bfs_roots =
      (int64_t *)xmalloc(((size_t)num_bfs_roots) * sizeof(int64_t));
  ...
  double make_graph_start = WTIME();
  {
    ...
    if (cart_comm != NULL) {
...
#endif
      has_edge =
          (unsigned char *)xMPI_Alloc_mem(((size_t)bitmap_size_in_bytes));
      ...
      for (block_idx = 0; block_idx < block_limit; ++block_idx) {
         ...
         * (int)block_limit); */
        FILE_Offset start_edge_index = int64_min(
            FILE_CHUNKSIZE * (block_idx * nrows + my_row), tg.nglobaledges);
        FILE_Offset edge_count =
...
#pragma omp atomic
    ...
    /* Find roots and max used vertex */
    {
      ...
      for (bfs_root_idx = 0; bfs_root_idx < num_bfs_roots; ++bfs_root_idx) {
        ...
        while (1) {
          ...
          counter += 2;
          if (counter > (2 * ((uint64_t)nglobalverts)))
            break;
          int is_duplicate = 0;
          int i;
   ...
   * used directly. */
  double *edge_counts =
      (double *)xmalloc(((size_t)num_bfs_roots) * sizeof(double));
  ...
  int64_t *pred =
      (int64_t *)xMPI_Alloc_mem(((size_t)nlocalverts) * sizeof(int64_t));
    ...
    if (!getenv("SKIP_VALIDATION")) {
...
#ifdef USE_DEBUG
      printf("before validate_bfs_result: max_used_vertex %ld, nlocalverts "
             "%ld, root %ld, pred[0] %ld\n",
             (long)max_used_vertex, (long)nlocalverts, (long)root,
             (long)pred[0]);
#endif

      int validation_passed_one = validate_bfs_result(
      ...
      if (rank == 0)
        fprintf(stderr, "TEPS for BFS %d is %g\n", bfs_root_idx,
                (double)edge_visit_count / bfs_times[bfs_root_idx]);
